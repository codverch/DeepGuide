...
// Callback function for performance monitoring
...
// Initialize and start the Redis server
int redis_wrapper_init_and_start_server() {
    ...
    // Run performance monitoring thread
...
int redis_wrapper_open_connection(const char* client_ip,
...
// Register Redis functions with the server
int redis_wrapper_register_functions(int (*set)(struct CallHandler,
                                                struct SetRequest,
                                                struct SetResponse*),
...
// Run a new listening thread
int redis_wrapper_run_new_listening_thread(int pin_cpu) {
...
int redis_wrapper_set_lb(int lb) {
    server.set_lb(lb);
    return 0;
}
