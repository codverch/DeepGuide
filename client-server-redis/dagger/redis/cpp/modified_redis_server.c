...
#include "/home/deepanjali/Dagger/sw/apps/redis/cpp/redis_cpp_wrapper.h"
...
/* init Dagger RPC */
int r = redis_wrapper_init_and_start_server();
if (r != 0)
    return r;

/* register RPC functions */
r = redis_wrapper_register_functions(&setCommand,
                                     &getCommand);
if (r != 0)
    return r;

int ti;
for (ti=0; ti<server.io_threads_num; ++ti) {
    /* open connections */
    r = redis_wrapper_open_connection("128.105.145.101", 3136, ti);
    if (r != 0)
        return r;

    /* run listening threads */
    r = redis_wrapper_run_new_listening_thread(12+ti);
    if (r != 0)
        return r;
}
